...
# $PROG: gfw3proxy.sh
# $description: install wall.cross & 3proxy 
# Public domain use as your own risk!
# Test on debian 8 jessie x86_64 Vmware VPS
# You must execute wall.cross by manual 
# input: wall.cross/server.sh after script finished. 
# $date: May 09, 2016
...
gfw_press_install() {
	...
	[ $? -eq 0 ] && cd wall.cross || {
		echo "Clone wall.cross failed.exiting..." >&2 ;
		exit 1 ;
	}
	pw=$(genpass)
	port=$(genport)
